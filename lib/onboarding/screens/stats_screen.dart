  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        const Spacer(),
        // Stats text - bottom aligned, right above button
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 32),
          child: RichText(
            textAlign: TextAlign.center,
            text: TextSpan(
              ...
              children: [
                ...
                TextSpan(
                  text: '4×',
                  style: OnboardingTheme.headingMedium.copyWith(
                    fontWeight: FontWeight.bold,
                  ),
                ),
                const TextSpan(
                    text: ' more peace within weeks of consistent reflection.'),
              ],
            ),
          ),
        ),
...
class StillnessGraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = 0; x <= size.width; x += 1) {
      ...
      final y =
          size.height * 0.85 - (size.height * 0.7 * math.pow(progress, 1.5));
      if (x == 0) {
    ...
    for (double x = 0; x <= size.width; x += 1) {
      ...
      final y =
          size.height * 0.7 + (size.height * 0.1 * math.pow(progress, 1.2));
      if (x == 0) {
  ...
  void _drawDashedLine(Canvas canvas, Offset start, Offset end, Paint paint) {
    ...
    for (int i = 0; i < dashCount; i++) {
      ...
      double startY =
          start.dy + ((end.dy - start.dy) / distance) * (totalDash * i);

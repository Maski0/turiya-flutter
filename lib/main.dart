...
class _MainScreenState extends State<_MainScreen>
  ...
  bool _isAgentConnecting = false; // Track if agent is connecting
  ...
  // Audio state removed - LiveKit handles all audio via PCM data channel
  ...
  /// Connect to LiveKit if user is already authenticated (for app restarts)
  Future<void> _connectLiveKitIfAuthenticated() async {
    try {
      final user = supabase.Supabase.instance.client.auth.currentUser;
      if (user == null) {
        print('⏭️ No user logged in yet, skipping LiveKit connection');
        return;
      }

      final cachedThreadId = await cacheService.getLastThreadId();
      if (cachedThreadId == null) {
        print('⏭️ No cached thread ID, skipping LiveKit connection');
        return;
      }

      print('🔌 User already authenticated, connecting to LiveKit...');
      print('   Thread ID: $cachedThreadId');
  ...
  Future<void> _loadLastThread() async {
    try {
      ...
      print('👤 User ID: ${user.id}');
      const avatarId =
          '12a65bd0-d264-479d-a5a4-ce0bdabdbcf9'; // Krishna's avatar ID
            ...
            if (state is ChatLoaded) {
              ...
              if (!state.isGenerating) {
                if (mounted && (_isGenerating || _showUserMessageBubble)) {
                  ...
                  print('✅ Synced state: _isGenerating=false');
                }
              }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        BottomInputBar(
                          ...
                          isAudioPlaying: false, // Audio handled by LiveKit
                          isConnecting: _isAgentConnecting,
